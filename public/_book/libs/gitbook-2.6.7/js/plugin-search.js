gitbook.require(['gitbook', 'lodash', 'jQuery'], function (gitbook, _, $) {
  ...
  var $highlighted = [],
    hi,
    hiOpts = { className: 'search-highlight' }
  var collapse = false,
    toc_visible = []
  ...
  function fetchIndex() {
    return $.getJSON(gitbook.state.basePath + '/search_index.json').then(
      loadIndex,
    ) // [Yihui] we need to use this object later
  }
  ...
  function search(q) {
    ...
    switch (_search.engine) {
      ...
      default:
        if (!index) return
        results = _.chain(index.search(q))
          .map(function (result) {
  ...
  function createForm(value) {
    ...
    $searchInput = $('<input>', {
      ...
      title:
        'Use Enter or the <Down> key to navigate to the next match, or the <Up> key to the previous match',
    })
  ...
  gitbook.events.bind('start', function (e, config) {
    ...
    // Type in search bar
    $(document)
      .on('keyup', '.book-search input', function (e) {
