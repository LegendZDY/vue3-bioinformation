gitbook.require(['gitbook', 'lodash', 'jQuery'], function (gitbook, _, $) {
  ...
  gitbook.events.bind('start', function (e, config) {
    ...
    if (down)
      if (down.length === 1 && /[.]pdf$/.test(down[0][0])) {
        gitbook.toolbar.createButton({
    ...
    var info = [
      'Keyboard shortcuts (<> indicates arrow keys):',
      '<left>/<right>: navigate to previous/next page',
      's: Toggle sidebar',
    ]
    if (config.search !== false)
      info.push(
        'f: Toggle search input ' +
    ...
    var li = $('a[href^="' + href + location.hash + '"]')
      .parent('li.chapter')
      .first()
    var summary = $('ul.summary'),
      chaps = summary.find('li.chapter')
    ...
    if (toc && toc.scroll_highlight !== false && li.length > 0)
      (function () {
        ...
        var hs = bookInner.find('.page-inner').find('h1,h2,h3'),
          n = hs.length,
          ts = hs.map(function (i, el) {
            return $(el).text()
          })
        if (n === 0) return
  ...
  var bookBody = $('.book-body'),
    bookInner = bookBody.find('.body-inner')
  ...
  $(function (e) {
    ...
    if ((pos && pos.focused) || !inIFrame())
      bookInner.find('.page-wrapper').focus()
